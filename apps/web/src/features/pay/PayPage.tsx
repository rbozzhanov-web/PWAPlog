...
import { parseTaxableYtd, ytdOverridesFor } from './ytdOverride';
...
export function PayPage({ db }: PayPageProps) {
  ...
  const result = payMonth && rate > 0 ? calculatePayPeriod(sectors, payMonth, settings, { [payMonth]: rate }, { [payMonth]: payDays }, undefined, ytdOverridesFor(payMonth, taxableYtd)) : undefined;
